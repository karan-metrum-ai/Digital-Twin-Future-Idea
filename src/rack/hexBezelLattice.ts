...
export function createHexBezelLattice(THREE, M, u) {
  ...
  const TW = 256, TH = Math.round(TW * Math.sqrt(3) / 3);
  ...
  const mk = (fill) => { const c = document.createElement('canvas'); c.width = TW; c.height = TH; const ctx = c.getContext('2d'); const im = ctx.createImageData(TW, TH); fill(im.data); ctx.putImageData(im, 0, 0); const t = new THREE.CanvasTexture(c); t.wrapS = t.wrapT = THREE.RepeatWrapping; t.anisotropy = 4; return t; };
  ...
  const mat = M('bezel_hex_lattice_' + u + 'u', { map: color, alphaMap: alphaT, normalMap: normal, normalScale: new THREE.Vector2(1, 1), roughness: 0.85, metalness: 0.05, color: 0x7c7c78, transparent: true, alphaTest: 0.5, side: THREE.FrontSide });
