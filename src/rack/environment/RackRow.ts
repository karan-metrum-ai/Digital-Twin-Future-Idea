// @ts-nocheck
/* eslint-disable */
// Two-row hot-aisle layout: five racks facing +z (row A, our interactive rack in the middle) and five racks
// rotated 180° (row B) so the two rows stand back-to-back with their rears facing each other across a contained
// hot aisle. The nine non-interactive racks are static replicas of the procedural rack: every mesh of the built
// rack is baked into world space and merged per material, so a whole replica costs a few dozen draw calls instead
// of several thousand. They share geometry buffers' source materials (so LED blink / thermal swap still apply)
// but carry none of the interactive plumbing (doors, exploded view, cabling toggles, liquid hoses).
...
export function buildRackReplicas(THREE, rack) {
  ...
  for (const { mat, geos } of buckets.values()) {
    ...
    if (!merged) continue;
    const m = new THREE.Mesh(merged, mat); m.name = 'replica_' + (mat.name || 'part'); m.castShadow = true; m.receiveShadow = true;
    template.add(m);
  }
  // Place nine copies (row A minus the interactive rack in slot 2, plus all of row B); copies share the merged
  // geometry buffers with the template, so the memory cost is one baked rack.
  for (const row of ['A', 'B']) for (let i = 0; i < ROW_XS.length; i++) {
    if (row === 'A' && ROW_XS[i] === 0) continue;
