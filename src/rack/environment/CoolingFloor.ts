...
// Texture/material is built once and shared across all grill instances (live + 10 replicas).

let sharedCoolingFloorMat = null;

function getCoolingFloorMat(THREE) {
  if (sharedCoolingFloorMat) return sharedCoolingFloorMat;
  ...
  sharedCoolingFloorMat = new THREE.MeshStandardMaterial({ map: tex, roughness: 0.75, metalness: 0.18 });
  return sharedCoolingFloorMat;
}

export function buildCoolingFloor(THREE) {
  const grp = new THREE.Group(); grp.name = 'cooling_floor';
  const mat = getCoolingFloorMat(THREE);
  ...
  const glow = new THREE.Mesh(
    new THREE.PlaneGeometry(0.68, 1.24),
    new THREE.MeshBasicMaterial({ color: 0x2f8dff, transparent: true, opacity: 0.06, depthWrite: false }),
  );
  glow.name = 'cooling_floor_glow';
  glow.rotation.x = -Math.PI / 2;
  glow.position.set(0, 0.0006, 0.64 + tileD + gap / 2 - 0.05);
  grp.add(glow);
