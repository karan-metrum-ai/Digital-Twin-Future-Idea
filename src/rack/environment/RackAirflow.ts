// @ts-nocheck
/* eslint-disable */
// Airflow + heat simulation for the nine non-interactive racks: each gets its own intake/exhaust particle sim
// (HeatSim), its own cold-air vapour rising off its own raised-floor grille (CoolingVapor + CoolingFloor), built
// once in "rack-local" coordinates (identical to how the interactive rack's own systems are built at the origin)
// and then instanced at every other rack's placement via a wrapper group — the same position/rotation transform
// `RackRow.buildRackReplicas` uses for the visual geometry. Particle counts are scaled down per replica (they're
// numerous and mostly read at a glance from a distance) to keep the combined point budget reasonable.
...
const REPLICA_DENSITY = 0.35; // fraction of the interactive rack's particle count, per replica
...
export function buildRackAirflowReplicas(THREE, slots) {
  ...
  for (const row of ['A', 'B']) for (let i = 0; i < ROW_XS.length; i++) {
    if (row === 'A' && ROW_XS[i] === 0) continue; // the interactive rack already builds its own heat/vapor/floor
